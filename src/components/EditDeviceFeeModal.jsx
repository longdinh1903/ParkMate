...
export default function EditDeviceFeeModal({ open, onClose, onUpdated, fee }) {
  ...
  useEffect(() => {
    if (fee) {
      setFormData({
        deviceType: fee.deviceType || "",
        deviceFee: fee.deviceFee || "",
        validFrom: fee.validFrom ? fee.validFrom.split("T")[0] : "",
        validUntil: fee.validUntil ? fee.validUntil.split("T")[0] : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await deviceFeeApi.update(fee.id, payload);
      showSuccess("Cập nhật cấu hình phí thiết bị thành công!");
      onUpdated();
    } catch (err) {
      console.error("Error updating device fee:", err);
      const serverMsg =
        err?.response?.data?.message || err?.response?.data || err.message;
      showError(serverMsg || "Cập nhật cấu hình phí thiết bị thất bại");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl relative overflow-hidden transform transition-all duration-300">
        ...
        {/* Form */}
        <form id="edit-device-fee-form" onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[calc(90vh-180px)] overflow-y-auto">
          {/* Device Type */}
          <div>
